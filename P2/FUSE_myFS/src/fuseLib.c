...
static int my_unlink(const char* path)
{
	...
	int nodeID = findFileByName(&myFileSystem, fname);
 ...
 * @return 0 on success and <0 on error
 **/
static int my_read(const char* path, void* read, size_t size, off_t offset, struct fuse_file_info* info)
{
	char buffer[BLOCK_SIZE_BYTES];
